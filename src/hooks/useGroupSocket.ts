...
export const useGroupSocket = ({
  ...
  const setupSocket = useCallback(async () => {
    if (!authToken || isSocketDisabled || !chatId || !currentUserId) {
      const reason = !authToken ? 'No auth token' : 
                    isSocketDisabled ? 'Socket disabled' :
                    !chatId ? 'No chatId' : 'No currentUserId';
    ...
    try {
      ...
      const socket = io(`${API_BASE_URL}/groupchat`, {
      ...
      console.log('🔌 [GroupSocket] Connection URL:', `${API_BASE_URL}/groupchat`);
      ...
      socket.on('receiveMessage', (newMessage: Message) => {
        ...
        const messageChatId = typeof newMessage.chat === 'object' ? (newMessage.chat as any)._id : newMessage.chat;
      ...
      // Group typing events
      socket.on('userTypingInGroup', async ({ userId, chatId: typingChatId }: { 
        userId: string, 
        chatId: string 
      }) => {
        console.log('⌨️ [GroupSocket] User typing event:', { userId, typingChatId, currentChatId: chatId });
      ...
      socket.on('userStopTypingInGroup', ({ userId, chatId: typingChatId }: { 
        userId: string, 
        chatId: string 
      }) => {
      ...
      socket.on('groupMemberRemoved', (data: { chatId: string; removedUserId: string; removedBy: any }) => {
  ...
  }, [authToken, chatId, currentUserId, isScreenActive, onNewMessage, onMessageRead, onMessageRevoked, onGroupMemberAdded, onGroupMemberRemoved, onGroupInfoUpdated, connectionAttempts, isSocketDisabled, joinedRoom]);
  ...
  useEffect(() => {
    ...
    const initSocket = async () => {
      if (!authToken || !chatId || !currentUserId || isUnmounted) {
        const reason = !authToken ? 'No auth token' : 
                      !chatId ? 'No chatId' : 
                      !currentUserId ? 'No currentUserId' : 'Unmounted';
