...
export const useGroupMessageOperations = ({ groupInfo, currentUserId }: UseGroupMessageOperationsProps) => {
  ...
  const loadMessages = useCallback(async (chatId: string, pageNum: number = 1, append: boolean = false) => {
    console.log('🔗 [GroupMessageOps] Loading messages for group:', chatId, 'page:', pageNum, 'append:', append);
    ...
    try {
      ...
      if (processedMessages.length > 0) {
        console.log('🔍 [GroupMessageOps] Sample message keys:', Object.keys(processedMessages[0]));
        const replyMessages = processedMessages.filter(m => m.replyTo);
        if (replyMessages.length > 0) {
          console.log('🔍 [GroupMessageOps] Sample reply message:', JSON.stringify(replyMessages[0], null, 2));
        }
      }
  ...
  const handleLoadMore = useCallback(() => {
    ...
    if (!hasMoreMessages || isLoadingMore || !groupInfo?._id) {
      console.log('⏭️ [GroupMessageOps] Skip load more:', { hasMoreMessages, isLoadingMore, hasGroupId: !!groupInfo?._id });
      return;
    }
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      console.log('📨 [GroupMessageOps] Server response headers:', response.headers.get('content-type'));
      ...
      console.log('🔍 [GroupMessageOps] Full message response:', JSON.stringify(newMessage, null, 2));
  ...
  const uploadAttachment = useCallback(async (file: any, type: 'image' | 'file') => {
    ...
    try {
      ...
      try {
        const testResponse = await fetch(`${API_BASE_URL}/api/chats/list`, {
          method: 'GET',
      ...
      uploadFormData.append('file', {
        ...
        type: file.mimeType || file.type || (type === 'image' ? 'image/jpeg' : 'application/octet-stream'),
      } as any);

      console.log('📤 [GroupMessageOps] Trying upload-attachment endpoint first...');
      
      let response = await fetch(`${API_BASE_URL}/api/chats/upload-attachment`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok) {
        console.log(`🔄 [GroupMessageOps] upload-attachment failed (${response.status}), trying message endpoint...`);
        ...
        response = await fetch(`${API_BASE_URL}/api/chats/message`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
      ...
      if (!response.ok) {
        ...
        if (isJson) {
          ...
          const textResponse = await response.text();
          console.error('Upload server returned non-JSON response:', textResponse.substring(0, 200));
  ...
  const uploadMultipleAttachments = useCallback(async (files: any[], type: 'image' | 'file') => {
    ...
    try {
      ...
      console.log('📤 [GroupMessageOps] Using upload-multiple endpoint for', files.length, 'files');
      ...
      files.forEach((file, index) => {
        const fileInfo = {
          uri: file.uri,
          name: file.fileName || file.name || `${type}_${index}.${type === 'image' ? 'jpg' : 'bin'}`,
          type: file.mimeType || file.type || (type === 'image' ? 'image/jpeg' : 'application/octet-stream'),
        };
      ...
      const response = await fetch(`${API_BASE_URL}/api/chats/upload-multiple`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'multipart/form-data',
      ...
      if (!response.ok) {
        ...
        if (response.status === 404 || response.status === 500) {
          console.log('🔄 [GroupMessageOps] upload-multiple not available, falling back to individual uploads...');
          const results: (Message | null)[] = [];
          
          for (let i = 0; i < files.length; i++) {
            const file = files[i];
            console.log(`📤 [GroupMessageOps] Uploading file ${i + 1}/${files.length}:`, file.name || file.uri);
          ...
          console.log(`✅ [GroupMessageOps] Successfully uploaded ${successfulUploads.length}/${files.length} files (individual)`);
        ...
        if (isJson) {
          try {
            ...
            const textResponse = await response.text();
            console.error('Multiple upload server response (HTML/Text):', textResponse.substring(0, 200));
          }
        } else {
          const textResponse = await response.text();
          console.error('Multiple upload server returned non-JSON response:', textResponse.substring(0, 200));
          errorMessage = 'Server trả về lỗi upload nhiều file không mong đợi. Vui lòng thử lại sau.';
        }
      ...
      Alert.alert('Lỗi', 
        error instanceof Error ? error.message : 'Có lỗi xảy ra khi tải lên nhiều file. Vui lòng thử lại.'
      );
      return [];
    }
  ...
  const handleMessageRead = useCallback(({ userId, chatId: updatedChatId }: { userId: string; chatId: string }) => {
  ...
  const handleMessageRevoked = useCallback(({ messageId, chatId: updatedChatId }: { messageId: string; chatId: string }) => {
    if (updatedChatId === groupInfo?._id) {
      setMessages(prev => prev.map(msg =>
        msg._id === messageId
          ? { 
