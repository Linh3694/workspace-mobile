...
export const useMessageOperations = ({ chat, currentUserId }: UseMessageOperationsProps) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (validMessages.length !== responseMessages.length) {
          console.warn(`Filtered out ${responseMessages.length - validMessages.length} invalid messages`);
        }
  ...
  const tryAlternativeEndpoints = useCallback(async (originalBody: any, token: string) => {
    const endpoints = [
      `${API_BASE_URL}/api/chat/message`,  // Alternative endpoint 1
      `${API_BASE_URL}/api/messages`,      // Alternative endpoint 2  
      `${API_BASE_URL}/api/chats/send-message`, // Alternative endpoint 3
    ];
  ...
  // Gửi tin nhắn
  const sendMessage = useCallback(async (content: string = '', emojiParam?: CustomEmoji, replyToId?: string) => {
  ...
  const markMessagesAsRead = useCallback(async (chatId: string | null, userId: string, token: string) => {
    if (!chatId) return;
  ...
  const revokeMessage = useCallback(async (messageId: string) => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/chats/message/${messageId}/revoke`, {
        method: 'DELETE',
        headers: {
