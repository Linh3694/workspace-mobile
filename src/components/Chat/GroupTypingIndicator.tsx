...
const GroupTypingIndicator: React.FC<GroupTypingIndicatorProps> = ({ typingUsers }) => {
  ...
  useEffect(() => {
    if (typingUsers.length > 0) {
      console.log(
        '⌨️ [GroupTypingIndicator] Showing typing indicator for:',
        typingUsers.map((u) => u.fullname)
      );
  ...
  const getTypingText = () => {
    console.log('🔍 [GroupTypingIndicator] getTypingText called with:', {
      ...
      users: typingUsers.map((u) => ({
        id: u._id,
        name: u.fullname || u.full_name || u.name || 'Unknown',
      })),
    });

    if (typingUsers.length === 1) {
      const userName =
        typingUsers[0].fullname || typingUsers[0].full_name || typingUsers[0].name || 'Người dùng';
      const result = `${userName} đang soạn tin...`;
      ...
      const userName =
        typingUsers[0].fullname || typingUsers[0].full_name || typingUsers[0].name || 'Người dùng';
      const result = `${userName} và ${typingUsers.length - 1} người khác đang soạn tin...`;
