                ...
                {message.replyTo && (
                  <View
                    ...
                    <TouchableOpacity
                      ...
                      {message.replyTo.type === 'multiple-images' && message.replyTo.fileUrls && (
                        <View style={{ marginTop: 4 }}>
                          <ImageGrid
                            ...
                            onPress={(index) =>
                              onImagePress && onImagePress(message.replyTo.fileUrls || [], index)
                            }
                          />
                        </View>
                      )}
                      ...
                      {/* Hiển thị text */}
                      {message.replyTo.type !== 'image' &&
                        message.replyTo.type !== 'multiple-images' &&
                        message.replyTo.type !== 'file' && (
                          <Text
                            style={{
                              fontSize: 14,
                              ...
                              if (!reaction.isCustom) {
                                ...
                                return (
                                  <Image
                                    key={idx}
                                    source={
                                      typeof emoji.url === 'string' ? { uri: emoji.url } : emoji.url
                                    }
                    ...
                    {(() => {
                      ...
                      if (isGroupChat) {
                        const otherParticipants =
                          chat?.participants?.filter((user) => user._id !== currentUserId) || [];
                        ...
                        const otherParticipants =
                          chat?.participants?.filter((user) => user._id !== currentUserId) || [];
                        const hasBeenRead = otherParticipants.some((user) =>
                          readByArray.includes(user._id)
                        );
