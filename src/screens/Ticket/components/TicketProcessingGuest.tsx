...
// Store & Hooks
import { useTicketData, useTicketActions, useTicketSubTasks } from '../../../hooks/useTicketStore';
import { acceptFeedback, updateTicket, getTeamMemberFeedbackStats } from '../../../services/ticketService';
...
const TicketProcessingGuest: React.FC<TicketProcessingGuestProps> = ({ ticketId }) => {
  ...
  const [technicianStats, setTechnicianStats] = useState<{ averageRating: number; totalFeedbacks: number } | null>(null);
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      setSubmitting(true);
      console.log('[handleSubmitFeedback] Submitting feedback:', { ticketId, rating, review, selectedBadges });
  ...
  return (
    <KeyboardAvoidingView
      ...
      <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
        ...
        <View className="rounded-full bg-white px-4 py-2">
          <Text
            ...
            {rawStatus === 'waiting for customer' ? 'Đang xử lý' : getStatusLabel(ticket.status)}
          </Text>
        </View>
      </View>
          ...
          {selectedOption === 'accept' && (
            <View>
              <View className="mt-4 rounded-2xl bg-[#f8f8f8] p-4">
                <Text className="mb-4 text-center font-bold text-base text-[#002855]">
                ...
                {/* Feedback Badges */}
                <Text className="mb-2 text-center font-semibold text-sm text-[#002855]">
      ...
      {status === 'closed' && ticket.feedback && (
        <View className="rounded-xl bg-gray-50 p-4">
          <Text className="mb-4 text-center font-bold text-base text-[#002855]">Phản hồi của bạn</Text>
