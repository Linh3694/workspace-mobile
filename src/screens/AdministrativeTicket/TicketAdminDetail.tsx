...
const TicketAdminDetail = () => {
  ...
  // Global store
  const { ticket, loading, error } = useAdministrativeTicketData();
  const { fetchTicket, assignToMe, assignToUser, cancelTicket } = useAdministrativeTicketActions();
  const {
  ...
  const reset = useAdministrativeTicketStore((state) => state.reset);
  ...
  const handleAssignToMe = async () => {
    ...
    if (success) {
      ...
      toast.error(getAdministrativeActionError('Không thể nhận ticket'));
    }
  };

  const handleAssignToUser = async (member: AdministrativeSupportMember) => {
    ...
    if (success) {
      ...
      toast.error(getAdministrativeActionError('Không thể chuyển ticket'));
    }
  };
