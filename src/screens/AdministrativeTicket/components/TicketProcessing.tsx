...
// Admin — giá trị API Frappe (khớp web Ticket Hành chính)
const ADMIN_STATUS_OPTIONS: AdministrativeTicketStatusUi[] = ['In Progress', 'Done', 'Cancelled'];
  ...
  const updateStatusAPI = async (newStatus: string, reason = '') => {
    try {
      ...
      if (newStatus === 'Cancelled' && reason) {
        const cancelTicketFn = useAdministrativeTicketStore.getState().cancelTicket;
  ...
  return (
    <KeyboardAvoidingView
      ...
      <ScrollView
        ...
        <View
          ...
          <View style={{ width: '100%' }}>
            <TouchableOpacity
              ...
              <Text style={{ fontSize: 16 }}>{getAdminTicketStatusLabel(selectedStatus)}</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        {/* Tin nhắn trao đổi — cùng ý nghĩa với ticket IT nhưng status theo API HC */}
        {showMessageThread && (
