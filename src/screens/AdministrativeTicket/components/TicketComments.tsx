...
interface GroupedMessage extends AdminTicketMessage {
  showHeader: boolean; // Whether to show avatar and name
}

// Gom tin nhắn liên tiếp cùng người gửi
const groupMessages = (messages: AdminTicketMessage[]): GroupedMessage[] => {
  if (!messages || messages.length === 0) return [];

  return messages.map((msg, index) => {
    const prevMsg = index > 0 ? messages[index - 1] : null;
    const sid = (s: { _id?: string; email?: string }) => s._id || s.email || '';
    const isSameSender = prevMsg && sid(prevMsg.sender) === sid(msg.sender);
...
const TicketComments: React.FC<TicketCommentsProps> = ({ ticketId }) => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const refreshed = await getAdminTicketMessages(ticketId);
